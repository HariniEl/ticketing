...
function PreBootcampTeam() {
    ...
    useEffect(()=>{
     ...
     const date = dd + '/' + (mm+1) + '/'+yy
        ...
        const length = querys.filter((len)=>(len.currentDate === date && len.team === "Pre-Bootcamp Team"))
     const processing = querys.filter((processing)=>(processing.status === 'processing' && processing.team === "Pre-Bootcamp Team"))
     const completed = querys.filter((completed)=>(completed.status === 'completed' && completed.team === "Pre-Bootcamp Team"))
...
<div className='listofquerysBefore450px'>
        {querys.map((n,index)=>(
  n.status !== 'pending' && n.team  === 'Pre-Bootcamp team'?
