...
function ZenClassTeam() {
    ...
    useEffect(()=>{
     ...
     const date = dd + '/' + (mm+1) + '/'+yy
        ...
        const length = querys.filter((len)=>(len.currentDate === date && len.team === "Zen-Class Team"))
     const processing = querys.filter((processing)=>(processing.status === 'processing'  && processing.team === "Zen-Class Team"))
     const completed = querys.filter((completed)=>(completed.status === 'completed'  && completed.team === "Zen-Class Team"))
...
<div className='listofquerysBefore450px'>
        {querys.map((n,index)=>(
  n.status !== 'pending' && n.team === 'Zen-Class Team'?
